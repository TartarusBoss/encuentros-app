...
export class Home {
  ...
  interesesDisponibles = ['académico', 'social', 'tecnología', 'cultural', 'networking', 'deportivo', 'videojuegos', 'música', 'mascotas', 'viajes', 'bienestar', 'voluntariado'];
  selectedInterests: string[] = ['académico', 'social'];
  aiLoading = false;
    ...
    this.http
      ...
      .subscribe({
        next: (res) => {
          ...
          this.computeMetrics();
          this.buscarEventosRecomendados();
        },
        error: (err) => {
  ...
  buscarEventosRecomendados() {
    const intereses = this.selectedInterests.length > 0 ? this.selectedInterests : ['académico', 'social'];
    const eventosParaIA = this.encuentros.map((e) => ({
      id: e.id,
      titulo: e.titulo,
      descripcion: e.descripcion || '',
      lugar: e.lugar || '',
      fecha: e.fecha ? new Date(e.fecha).toISOString() : '',
      categoria: this.inferEventCategory(e.titulo, e.descripcion || ''),
    }));

    if (eventosParaIA.length === 0) {
    ...
    this.encuentroService.getRecommendedEvents(intereses, eventosParaIA).subscribe({
      next: (res) => {
        const items = Array.isArray(res?.recomendaciones) ? res.recomendaciones : [];

        this.recommendedEvents = items
          .filter((item) => item && (item.id || item.titulo))
          .slice(0, 3)
          .map((item) => ({
            id: Number(item.id ?? 0),
            titulo: item.titulo || 'Evento recomendado',
            match: Number(item.match ?? 0),
            motivo: item.motivo || 'Coincide con tus intereses.',
          }));

        if (this.recommendedEvents.length === 0) {
          this.recommendedEvents = this.upcoming.slice(0, 3).map((e) => ({
            id: e.id,
            titulo: e.titulo,
            match: 80,
            motivo: 'Evento recomendado para ti por proximidad y relevancia.',
          }));
        }
  ...
  private inferEventCategory(titulo: string, descripcion = ''): string {
    const text = `${titulo} ${descripcion}`.toLowerCase();
    if (/tech|tecnolog|program|software|develop|hack|startup|code|ai|ia/.test(text)) {
      return 'tecnología';
    }
    if (/arte|cultural|musica|música|concierto|galeria|cine|diseño|festival/.test(text)) {
      return 'cultural';
    }
    if (/videojuego|juego|gaming|gamer|esports|esport|steam|consola/.test(text)) {
      return 'videojuegos';
    }
    if (/mascota|perro|gato|adopcion|rescate|pet|dog|cat/.test(text)) {
      return 'mascotas';
    }
    if (/viaje|turismo|excursion|trekking|senderismo|roadtrip|travel/.test(text)) {
      return 'viajes';
    }
    if (/bienestar|yoga|meditacion|mindfulness|fitness|salud|running|gym/.test(text)) {
      return 'bienestar';
    }
    if (/voluntariado|donacion|solidario|comunidad|caridad|apoyo/.test(text)) {
      return 'voluntariado';
    }
    if (/univers|acad|seminario|taller|clase|estudio|curso/.test(text)) {
      return 'académico';
    }
    return 'social';
  }
