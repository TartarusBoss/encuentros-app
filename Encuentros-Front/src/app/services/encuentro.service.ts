...
export class EncuentroService {
  ...
  /**
   * Recomendaciones de eventos usando IA local con Ollama
   */
  getRecommendedEvents(intereses: string[], eventos: Array<Record<string, any>>): Observable<{ recomendaciones: RecomendacionEvento[] }> {
    return this.http.post<{ recomendaciones: RecomendacionEvento[] }>(`${this.API_URL}/recomendaciones`, {
      intereses,
      eventos,
    });
  }
